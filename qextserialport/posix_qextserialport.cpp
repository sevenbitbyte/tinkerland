...
bool Posix_QextSerialPort::open(OpenMode mode)
{
    ...
    if (!isOpen()) {
        ...
        if (Posix_File->open(QIODevice::ReadWrite|QIODevice::Unbuffered)) {
            ...
            /*set open mode*/
            QIODevice::open(mode);
            if(Posix_File->handle() != -1){
            	if(Read_Notifier == NULL){
            		Read_Notifier=new QSocketNotifier(Posix_File->handle(), QSocketNotifier::Read, this);
            	}
            	Read_Notifier->setEnabled(true);
				connect(Read_Notifier, SIGNAL(activated(int)), SLOT(emitReadyRead()));
            }
...
qint64 Posix_QextSerialPort::writeData(const char * data, qint64 maxSize)
{
    ...
    UNLOCK_MUTEX();
    qDebug("writeData - Wrote %i bytes", retVal);
    flush();
    return retVal;
}
