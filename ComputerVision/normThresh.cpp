...
int main(int argc, char** argv){
	...
	cvNamedWindow("NormalizedFrame", CV_WINDOW_AUTOSIZE);
	cvSetMouseCallback("NormalizedFrame", mouseHandler, (void*)input);

	int level=1;

	cvCreateTrackbar("Red Target", "Controls", &redTarget, 100, forceUpdate);
	cvCreateTrackbar("Green Target", "Controls", &greenTarget, 100, forceUpdate);
	cvCreateTrackbar("Blue Target", "Controls", &blueTarget, 100, forceUpdate);
	...
	while(keyVal!='q'){
		...
		if(updateNeeded || !isPaused){
			...
			if(frameCount < STARTUP_DELAY){
				...
				if(normImg==NULL){
					normImg=cvCreateImage( cvGetSize(input), IPL_DEPTH_8U, 3 );
					cvSetZero(normImg);
				}
				...
				for(int row=0; row<input->height; row++){
					for(int col=0; col<input->width; col++){
						...
						float rDelta=r - ((float)redTarget)/100;
						float gDelta=g - ((float)greenTarget)/100;
						float bDelta=b - ((float)blueTarget)/100;
						float dist=sqrtf( (rDelta*rDelta) + (gDelta*gDelta) + (bDelta*bDelta) ) * 100;

						/*if( dist < maxDistance ){
							GetPixelPtrD8(maskImg, row, col, 0) = 255;
						}*/
				...
				cvThreshold(distImg, maskImg, (double) maxDistance, 255, CV_THRESH_BINARY_INV);
			...
			cvShowImage("NormalizedFrame", normImg);
...
void mouseHandler(int event, int col, int row, int flags, void* i){
	...
	if(event==CV_EVENT_LBUTTONDOWN){
		...
		cout<<endl<<"Red="<<red<<" Green="<<green<<" Blue="<<blue<<" Sum="<<sum<<endl;
		...
		/*if(dist < maxDistance){
			cout<<"TARGET"<< (int)((uint8_t) dist) << endl;
		}
		else{
			cout<<(int)((uint8_t) dist) << endl;
		}*/
		...
		cout<<"Updating to R="<<r<<" G="<<g<<" B="<<b<<endl;

		cvSetTrackbarPos("Red Target", "Controls", (int)(r*100));
		cvSetTrackbarPos("Green Target", "Controls", (int)(g*100));
		cvSetTrackbarPos("Blue Target", "Controls", (int)(b*100));
